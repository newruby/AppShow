...
public class KpiHandler2 {
	...
	public void calKpiScore(List<Double> list){
		...
		try {
			...
			while(rs.next()){
				...
				double a=acess_n/(c1+1);
				double a2=maintian_n/(c2+1);
				double a3=0.0;
				if(c3==2){
					a3=mobile_n/c3;
				}else{
				...
				double a4=capacity_n/(c4+1);
				double a5=integrity_n/(c5+1);
	...
	public static void main(String[] args) {
		...
		KpiHandler2 handler = new KpiHandler2();
		//将t_omc中数据归一化
	//	handler.normalize();
